...
error if \texttt{j < n}. It is also possible to specify an optional step size
using a third slice argument. As an example, the following snippet returns only
even numbers:

\begin{lstlisting}
let f(): i32[] = let x = iota 42
                  in x[0:41:2]
...
inference. As a concrete example, here is the definition of the Mandelbrot
set iteration step $Z_{n+1} = Z_{n}^{2} + C$, where $Z_n$ is the actual
iteration value, and $C$ is the initial point. In this example, all operations
on complex numbers are fully expanded, though Futhark comes with a library for
complex numbers that will be presented later.

\begin{lstlisting}
let mandelbrot_step(Zn: (f32, f32), C: (f32, f32)): (f32, f32) =
    let (Zn_r, Zn_i) = Zn
    let (C_r, C_i) = C
    let real_part = Zn_r*Zn_r - Zn_i*Zn_i + C_r
    let imag_part = 2f32*Zn_r*Zn_i + C_i
