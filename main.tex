...
\begin{lstlisting}[mathescape=true]
  ...
  $e$ ::= $v$  |  let $\id{ps}$ = $e$ in $e$
      ...
      | loop ($p_1$=$e_1$,$\cdots$,$p_n$=$e_n$) = for ($e$) = $e'$ in $e''$
...
\noindent We shall refer to the types \kw{i32}, \kw{f32}, \kw{bool} as
...
\twopart{
\fraccc{}{\vd \kw{true} : \kw{bool}}
}{
\fraccc{}{\vd \kw{false} : \kw{bool}}
}
...
\twopart{
  ...
  \mathrm{TypeOf}(\id{binop}) \geq \tau_1 \rarr \tau_2 \rarr \tau}{\Gamma \vd e_1 ~\id{binop}~ e_n : \tau
  }
}{
  \fraccc{\Gamma \vd e_i : \tau_i ~~ i = [1;n] \\
  \mathrm{TypeOf}(\id{op}) \geq \tau_1 \rarr \cdots \rarr \tau_n \rarr \tau}{\Gamma \vd \id{op}~ e_1~\cdots~e_n : \tau
  }
}
...
\onepart{
  \fraccc{\Gamma,x_1:\tau_1,\cdots,x_n:\tau_n \vd e : \tau  ~~~~~ f \not \in \Dom(\Gamma)}
         {\Gamma \vd \kw{fun}~f~\kw{(}x_1,\cdots,x_n\kw{)}~\kw{=} ~e : \{f:\kw{(}\tau_1,\cdots,\tau_n\kw{)} \rarr \tau\}}
}

\section{Futhark Evaluation Semantics}

In this section we develop a simple big-step semantics for Futhark
programs. Besides from the notion of value, which has already been
introduced in the previous section, we need a notion of \emph{dynamic
  environment}, which maps identifiers to values

\newcommand{\Eval}{\mathrm{Eval}}
\begin{eqnarray*}
  \Eval(P,v) & = & \Eval_p(\id{main}(v))
\end{eqnarray*}
...
\begin{lstlisting}[mathescape=true]
  ...
  $W(f$ $e_1 \cdots e_n)$ =
    $W(e_1) + \ldots + W(e_n) + W(e[v_1/x_1,\cdots v_n/x_n])$
    $\Where{P(f) = \lambda x_1 \cdots x_n . e \\
            v_1 = \Eval(e_1), \cdots, v_n = \Eval(e_n)}$
