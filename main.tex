...
Additionally, there is \fop{iota}, which creates an array of a
...
The name \fop{iota} comes from APL, one of the earliest array
programming languages, and is supposed to be mnemonic for creating
\textit{index spaces} of arrays.  Put another way, \fop{iota}\texttt{ n}
produces an array of valid indices into an array of size \texttt{n}.

The \fop{replicate} construct is used to create an array of some
...
to index the expression \fop{iota}\texttt{ 2} using \texttt{[1,2,3]} as the
...
Be aware that \fop{zip} requires all of the input arrays to have
...
\fop{zip} and \fop{unzip} also have no runtime cost (no copying
...
\fop{iota} is unlikely to go well:
...
We can use \fop{map} and \fop{iota} to duplicate many other
...
Using \fop{map}s over \fop{iota}s with explicit indexing is
...
The \fop{scan} SOAC in Futhark is inclusive, but it is of course
easy to generate a corresponding exclusive scan simply by prepending
the neutral element.

While the idea behind \fop{reduce} is probably familiar,
\fop{scan} is a little more esoteric, and mostly has applications
...
We have seen \fop{map}, which permits us to change all the elements
of an array.  We have seen \fop{reduce}, which lets us collapse all
...
\fop{filter}, which behaves much like a filter in any other
functional language:
...
\begin{wrap}
\lstinputlisting[lastline=4]{src/multable.fut}
\end{wrap}
...
\onepart{
  \fraccc{\Gamma,x_1:\tau_1~\cdots~x_n:\tau_n \vd e : \tau}{
    \Gamma \vd \backslash ~x_1:\tau_1~\cdots~x_n:\tau_n ~ \kw{->} ~e : \tau_1 \rarr \cdots \rarr \tau_n \rarr \tau
  }
}
...
\begin{wrap}
\lstinputlisting[firstline=11,lastline=15]{src/reduce_contract.fut}
\end{wrap}
...
\begin{wrap}
\lstinputlisting[firstline=2,lastline=8]{src/reduce_contract.fut}
\end{wrap}
...
\begin{figure}
\begin{wrap}
\lstinputlisting[firstline=17,lastline=31]{src/rsort.fut}
\end{wrap}
...
\begin{figure}
\begin{wrap}
\lstinputlisting[firstline=15]{src/primes.fut}
\end{wrap}
...
\begin{wrap}
\lstinputlisting[firstline=2,lastline=8]{src/sgm_scan.fut}
\end{wrap}
...
\begin{wrap}
\lstinputlisting[firstline=4,lastline=8]{src/find_idx.fut}
\end{wrap}
...
\begin{wrap}
\lstinputlisting[firstline=10,lastline=13]{src/find_idx.fut}
\end{wrap}
...
\lstinputlisting[lastline=8]{src/maxidx.fut}
...
\begin{figure}
\lstinputlisting[firstline=15,lastline=32]{src/rsort_idx.fut}
\caption{Generalised Ascending Version of Radix Sort.}
\label{fig:rsort_idx_asc}
\end{figure}

\begin{figure}
\lstinputlisting[firstline=34,lastline=50]{src/rsort_idx.fut}
...
\begin{wrap}
\lstinputlisting[firstline=21,lastline=34]{src/streak.fut}
\end{wrap}
...
\begin{wrap}
\lstinputlisting[firstline=28,lastline=35]{src/sgm_streak.fut}
\end{wrap}
...
\lstinputlisting[firstline=10,lastline=19]{src/sgm_repl.fut}
...
\begin{figure}
  \lstinputlisting[firstline=3,lastline=22]{src/lines_seq.fut}
...
\begin{figure}
  \lstinputlisting[firstline=24,lastline=45]{src/lines_seq.fut}
