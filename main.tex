...
simple performance reasoning principles of the von Neumann computing
...
\begin{enumerate}
\item Moore's law, CPUs, GPUs, other parallel architectures (OK)
\item Concurrency vs parallelism (OK)
\item Task parallelism, data parallism, simd, mimd (OK)
\item Low-level languages (CUDA, OpenCL) vs high-level languages (Futhark, NESL) vs annotation approaches (OpenAcc, OpenMP) vs library approaches (CUBLAS)
\item Cost models for machines and for languages - problems with the RAM model, problems with the PRAM model, Work and Span.
\end{enumerate}

In this book, we shall look at parallelism from a data-parallel
